...
async function fetchRealFeishuData(env: Env): Promise<WikiFundingRecord[]> {
  ...
  try {
    ...
    let pageNum = 0;
    const PAGE_SIZE = 100; // 每页100条,减少请求次数

    // 循环获取所有分页数据
    do {
      pageNum++;
      const url = `${FEISHU_BITABLE_CONFIG.BASE_URL}/bitable/v1/apps/${FEISHU_BITABLE_CONFIG.APP_TOKEN}/tables/${FEISHU_BITABLE_CONFIG.TABLE_ID}/records?page_size=${PAGE_SIZE}${pageToken ? `&page_token=${pageToken}` : ''}`;
      ...
      const data = await response.json();
      console.log(`📊 第${pageNum}页: 状态=${data.code}, 获取 ${data.data?.items?.length || 0} 条记录`);
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!企业介绍.trim() && !细分领域.trim() && !公司官网.trim()) {
          console.log(`⚠️ 记录 ${records.length + i + 1} 缺少关键信息，跳过`);
          continue;
        }

        const record: WikiFundingRecord = {
          id: item.record_id || `feishu_${records.length + i + 1}`,
          序号: fields['序号'] || (records.length + i + 1),
        ...
        records.push(record);
        console.log(`✅ 处理记录 ${records.length}: ${record.细分领域} - ${record.企业介绍.substring(0, 50)}...`);
      }

      // 获取下一页token
      pageToken = data.data?.page_token;

    } while (pageToken && pageToken.length > 0); // 有下一页就继续

    console.log(`🎉 共获取 ${pageNum} 页，成功解析 ${records.length} 条真实飞书创投日报记录`);
    return records;
