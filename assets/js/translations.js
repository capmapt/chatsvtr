// Translation configuration
const translations = {
  'zh-CN': {
    title: '硅谷科技评论',
    sidebar_title: '硅谷科技评论',
    nav_ai100: 'AI 100',
    nav_ranking: 'AI创投榜',
    sub_financing: '融资概览',
    sub_industries: '赛道概览',
    sub_startups: '公司概览',
    sub_investors: '机构概览',
    sub_people: '人员概览',
    nav_database: 'AI创投库',
    nav_group: 'AI创投群',
    nav_meetup: 'AI创投会',
    nav_camp: 'AI创投营',
    nav_about: '关于我们',
    sidebar_qr_text: '添加好友，进群交流',
    banner_title: '硅谷科技评论',
    banner_subtitle: 'SVTR.AI',
    banner_tagline_left: '全球前沿科技',
    banner_tagline_right: '创业投资孵化',
    notice_1: '100K+',
    link_community: '社区成员',
    notice_2: '10K+',
    link_alliance: '创业公司与投资机构',
    notice_3: '1K+',
    link_members: '权益会员',
    chat_header: '与 <a href="https://svtr.ai" target="_blank">SVTR.AI</a> 对话',
    tag_database_text: 'AI创投库',
    // 用户登录系统
    login_required: '登录后即可使用AI对话功能',
    login: '登录',
    logout: '注销',
    community_member: '社区成员',
    welcome_title: '欢迎加入硅谷科技评论',
    login_subtitle: '选择您偏好的登录方式',
    wechat_login: '微信登录',
    github_login: 'GitHub登录',
    or: '或',
    email_placeholder: '邮箱地址',
    password_placeholder: '密码',
    email_login: '邮箱登录',
    no_account: '还没有账号？立即注册',
    forgot_password: '忘记密码？',
    ai_chat_title: 'SVTR AI 智能助手',
    ai_chat_desc: '获取AI创投领域的专业insights和实时资讯',
    start_chat: '开始对话',
    tag_meetup_text: 'AI创投会',
    tag_camp_text: 'AI创投营',
    menu_toggle_aria_label: '打开/关闭菜单',
    logo_alt_text: 'SVTR.AI 商标',
    qr_alt_text: '添加好友二维码',
    join_btn: '加入候补名单',
    input_placeholder: '请输入你的问题...',
    send_btn: '发送',
    footer_text: '© 2025 SVTR.AI. All rights reserved.'
  },
  'en': {
    title: 'SVTR.AI',
    sidebar_title: 'SVTR.AI',
    nav_ai100: 'AI 100',
    nav_ranking: 'AI Ranking',
    sub_financing: 'Financing',
    sub_industries: 'Industries',
    sub_startups: 'Startups',
    sub_investors: 'Investors',
    sub_people: 'People',
    nav_database: 'AI Database',
    nav_group: 'AI Group',
    nav_meetup: 'AI Meetup',
    nav_camp: 'AI Camp',
    nav_about: 'About',
    sidebar_qr_text: 'Scan QR to join chat',
    banner_title: 'Silicon Valley<br>Technology Review',
    banner_subtitle: 'SVTR.AI',
    banner_tagline_left: 'Global Frontier‑Tech',
    banner_tagline_right: 'Venture Incubation',
    notice_1: 'If you are an AI Practitioner, welcome to join SVTR.AI',
    link_community: 'Community',
    notice_2: 'If you are an AI Founder, welcome to join SVTR.AI',
    link_alliance: 'Ecosystem Alliance',
    notice_3: 'If you are a Member, please visit SVTR.AI',
    link_members: 'Exclusive Members Area',
    chat_header: 'Chat with <a href="https://svtr.ai" target="_blank">SVTR.AI</a>',
    tag_database_text: 'AI DB',
    // User login system
    login_required: 'Login to use AI chat features',
    login: 'Login',
    logout: 'Logout',
    community_member: 'Community Member',
    welcome_title: 'Welcome to SVTR.AI',
    login_subtitle: 'Choose your preferred login method',
    wechat_login: 'WeChat Login',
    github_login: 'GitHub Login',
    or: 'or',
    email_placeholder: 'Email address',
    password_placeholder: 'Password',
    email_login: 'Email Login',
    no_account: 'No account? Sign up now',
    forgot_password: 'Forgot password?',
    ai_chat_title: 'SVTR AI Assistant',
    ai_chat_desc: 'Get professional insights and real-time news in AI venture capital',
    start_chat: 'Start Chat',
    tag_meetup_text: 'AI Meetup',
    tag_camp_text: 'AI Camp',
    menu_toggle_aria_label: 'Open/Close menu',
    logo_alt_text: 'SVTR.AI Logo',
    qr_alt_text: 'Add friend QR code',
    join_btn: 'Join the Waitlist',
    email_placeholder: 'Enter your email',
    input_placeholder: 'Type your question...',
    send_btn: 'Send',
    footer_text: '© 2025 SVTR.AI. All rights reserved.'
  }
};

// Export for module usage
if (typeof module !== 'undefined' && module.exports) {
  module.exports = translations;
}
