  ...
  constructor(){
    ...
    if(translations[t]){
      if(this.currentLang=t,document.documentElement.lang=t,this.updateSEOMetaTags(t),document.querySelectorAll('[data-i18n]').forEach(e=>{
        const n=e.getAttribute('data-i18n'),a=translations[t][n]||e.textContent;this.keysForInnerHTML.includes(n)?e.innerHTML=a:e.textContent=a,'banner_title'===n&&e.setAttribute('data-lang',t);
        ...
        for(const n of e.attributes){
          if(n.name.startsWith('data-i18n-attr-')){
            const a=n.name.substring(15),i=n.value;translations[t][i]&&e.setAttribute(a,translations[t][i]);
          }
        }
    ...
    }const n=document.querySelector('meta[property="og:title"]'),a=document.querySelector('meta[property="og:description"]'),i=document.querySelector('meta[name="twitter:title"]'),r=document.querySelector('meta[name="twitter:description"]');n&&e.title&&n.setAttribute('content',e.title),a&&e.description&&a.setAttribute('content',e.description),i&&e.title&&i.setAttribute('content',e.title),r&&e.description&&r.setAttribute('content',e.description);const s=document.querySelector('link[rel="canonical"]');if(s){
    ...
    const e=document.querySelector('.qr-container');if(!e){
      return;
    }e.setAttribute('data-lang',t);const n=e.querySelector('.qr-image-wrapper'),a=e.querySelector('.qr-overlay'),i=e.querySelector('.discord-join-button');n&&n.setAttribute('data-lang',t),a&&a.setAttribute('data-lang',t),i&&i.setAttribute('data-lang',t);
  }getCurrentLanguage(){
