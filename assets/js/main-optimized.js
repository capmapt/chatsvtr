!function(){
  'use strict';const t=2e3,e=3e3,s=2e4,o=500,i=6e4,n={members:{count:121884,growth:25,max:15e4},companies:{count:10761,growth:8,max:2e4},vip:{count:1102,growth:3,max:2e3}};class a{
    constructor(){
      ...
      this.domElements.toggle=document.querySelector('.menu-toggle'),this.domElements.sidebar=document.querySelector('.sidebar'),this.domElements.overlay=document.querySelector('.overlay'),this.domElements.content=document.querySelector('.content'),this.domElements.stats={members_count:document.getElementById('members-count'),companies_count:document.getElementById('companies-count'),vip_count:document.getElementById('vip-count'),members_growth:document.getElementById('members-growth'),companies_growth:document.getElementById('companies-growth'),vip_growth:document.getElementById('vip-growth')},this.hasStatsElements=Object.values(this.domElements.stats).every(t=>null!==t);
    }initializeStats(){
      const t={};return Object.keys(n).forEach(e=>{
        t[e]={...n[e],last:Date.now()};
      }),t;
    }setupEventListeners(){
      this.domElements.toggle&&this.domElements.overlay&&(this.domElements.toggle.addEventListener('click',()=>this.toggleSidebar()),this.domElements.overlay.addEventListener('click',()=>this.closeSidebar())),document.addEventListener('keydown',t=>{
        'Escape'===t.key&&this.isSidebarOpen()&&this.closeSidebar();
      });
    }initializeSidebar(){
      const e=window.innerWidth<=768,s=!localStorage.getItem('sidebarAutoClosed');e?this.isSidebarOpen()&&(e||s)&&setTimeout(()=>{
        this.closeSidebar(),s&&localStorage.setItem('sidebarAutoClosed','1');
      },t):this.isSidebarOpen()||this.openSidebar();
    }toggleSidebar(){
      ...
      this.hasStatsElements&&(this.paintStats(),this.updateProgressBars(),this.timers.push(setInterval(()=>this.stepStats(),e),setInterval(()=>this.randomStatsUpdate(),s)));
    }formatNumber(t){
      ...
      const t=Date.now();let e=!1;Object.entries(this.stats).forEach(([s,o])=>{
        if(t-o.last>=i){
          const i=Math.max(1,Math.floor(o.growth*(1+.6*Math.random()-.3)));o.count+=i,o.last=t,e=!0,Math.random()<.2&&(o.growth=Math.max(1,Math.floor(o.growth*(1+.4*Math.random()-.2)))),this.flashElement(`${s}-count`);
        }
    ...
    window.svtrApp&&window.svtrApp.destroy(),window.svtrApp=new a;
  ...
  },250);window.addEventListener('resize',m),'loading'===document.readyState?document.addEventListener('DOMContentLoaded',r):r(),'localhost'!==window.location.hostname&&'127.0.0.1'!==window.location.hostname||(window.SVTRApp=a);
}();
