...
// .wrangler/tmp/bundle-0tPzOo/checked-fetch.js
...
// .wrangler/tmp/pages-pONbcG/functionsWorker-0.030831583435695142.mjs
...
var QueryExpansionService = class {
  static {
    __name(this, "QueryExpansionService");
  }
  static {
    __name2(this, "QueryExpansionService");
  }
  synonymMap;
  domainTerms;
  queryPatterns;
  ...
  generateRelatedTerms(keywords, queryType) {
    ...
    const typeMapping = {
      ...
      [
        "market_trends"
        /* MARKET_TRENDS */
      ]: ["market-analysis", "ai-technology"],
      [
        "technology_info"
        /* TECHNOLOGY_INFO */
      ]: ["ai-technology", "startup-evaluation"],
      [
        "funding_info"
        /* FUNDING_INFO */
      ]: ["investment", "startup-evaluation"],
  ...
  buildDomainContext(queryType, keywords) {
    ...
    context.push(...contextMap[queryType] || contextMap[
      "general"
      /* GENERAL */
    ]);
...
__name2(createQueryExpansionService, "createQueryExpansionService");
var SemanticCacheService = class {
  static {
    __name(this, "SemanticCacheService");
  }
  static {
    __name2(this, "SemanticCacheService");
  }
...
__name2(createSemanticCacheService, "createSemanticCacheService");
var WebSearchService = class {
  static {
    __name(this, "WebSearchService");
  }
  static {
    __name2(this, "WebSearchService");
  }
  searchApiKey;
  searchEngineId;
  fallbackEnabled;
  ...
  async performIntelligentSearch(query, options = {
    ...
    console.log(`\u{1F310} \u5F00\u59CB\u667A\u80FD\u7F51\u7EDC\u641C\u7D22: "${query}"`);
    const optimizedQuery = this.optimizeSearchQuery(query);
    console.log(`\u{1F3AF} \u4F18\u5316\u67E5\u8BE2: "${optimizedQuery}"`);
    try {
      ...
      const finalResults = verifiedResults.sort((a, b) => b.relevanceScore - a.relevanceScore).slice(0, options.maxResults);
  ...
  async googleCustomSearch(query, options) {
    ...
    const searchUrl = `https://www.googleapis.com/customsearch/v1?key=${this.searchApiKey}&cx=${this.searchEngineId}&q=${encodeURIComponent(query)}&num=${options.maxResults}&lr=lang_${options.language === "zh-CN" ? "zh" : "en"}`;
    try {
   ...
   * AI创投专业源搜索
   */
  async specializedAIVCSearch(query, options) {
   ...
   * 内容验证和评分
   */
  async verifyAndScoreResults(results, originalQuery) {
    const keywords = this.extractKeywords(originalQuery);
   ...
   */
  extractKeywords(query) {
    return query.toLowerCase().replace(/[^\w\s\u4e00-\u9fa5]/g, "").split(/\s+/).filter((word) => word.length > 2);
  }
  /**
   * 计算关键词匹配数
   */
...
function createWebSearchService(config = {}) {
  return new WebSearchService(config);
}
__name(createWebSearchService, "createWebSearchService");
__name2(createWebSearchService, "createWebSearchService");
var HybridRAGService = class {
  ...
  async performIntelligentRAG(query, options = {}) {
    ...
    console.log("\u{1F4AB} \u6267\u884C\u5B8C\u6574RAG\u68C0\u7D22 + \u7F51\u7EDC\u641C\u7D22...");
   ...
   * 加权关键词匹配 - 增强联系方式过滤
   */
  async findWeightedKeywordMatches(weightedKeywords, options = {}) {
    const documents = await this.getStoredDocuments();
    const matches = [];
    documents.forEach((doc) => {
      ...
      if (options.contactInfoQuery) {
        ...
        if (isThirdPartyCompany) {
          console.log(`\u{1F6AB} \u8FC7\u6EE4\u7B2C\u4E09\u65B9\u8054\u7CFB\u4FE1\u606F: ${doc.title}`);
          return;
        }
   ...
   */
  shouldUseWebSearch(queryExpansion, originalQuery) {
   ...
   * 执行网络搜索 - 增强版，更多结果和数据源
   */
  async performWebSearch(searchQuery, queryExpansion, options) {
    try {
      ...
      const ragMatches = webResults.map((result) => ({
      ...
      console.log(`\u{1F310} \u7F51\u7EDC\u641C\u7D22\u5B8C\u6210: ${ragMatches.length}\u4E2A\u7ED3\u679C`);
      return {
        matches: ragMatches,
        source: "web_search",
        isRealtime: true,
      ...
      console.log("\u{1F310} \u7F51\u7EDC\u641C\u7D22\u5931\u8D25:", error.message);
      return {
        matches: [],
...
// .wrangler/tmp/bundle-0tPzOo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0tPzOo/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.030831583435695142.js.map
