...
// .wrangler/tmp/bundle-5j1QCX/checked-fetch.js
...
// .wrangler/tmp/pages-cVgoM6/functionsWorker-0.23408269078356558.mjs
...
var init_checked_fetch = __esm({
  "../.wrangler/tmp/bundle-NbLIRh/checked-fetch.js"() {
    "use strict";
...
async function createOrUpdateGitHubUser(env, githubUser, email) {
...
function getCurrentDomain(request, env) {
...
__name(getCurrentDomain, "getCurrentDomain");
async function onRequestGet(context) {
  try {
    ...
    if (code) {
      try {
        let originalDomain2 = currentDomain;
        if (state) {
          try {
            const stateData2 = JSON.parse(atob(state));
            originalDomain2 = stateData2.originalDomain || currentDomain;
            console.log("GitHub OAuth\u56DE\u8C03\uFF0C\u539F\u59CB\u57DF\u540D:", originalDomain2);
          } catch (e) {
        ...
        if (!tokenResponse.ok) {
          ...
          return Response.redirect(`${originalDomain2}?auth_error=token_exchange_failed`);
        }
        ...
        if (!userResponse.ok) {
          ...
          return Response.redirect(`${originalDomain2}?auth_error=user_info_failed`);
        }
        ...
        console.error("GitHub OAuth\u56DE\u8C03\u5904\u7406\u5931\u8D25:", error2);
...
var init_github = __esm({
  "api/auth/github.ts"() {
    "use strict";
    init_functionsRoutes_0_39491537850192104();
    init_checked_fetch();
    __name2(getGitHubPrimaryEmail, "getGitHubPrimaryEmail");
    __name2(createOrUpdateGitHubUser, "createOrUpdateGitHubUser");
    __name2(createUserSession, "createUserSession");
    __name2(getCurrentDomain, "getCurrentDomain");
    __name2(onRequestGet, "onRequestGet");
    __name2(onRequestOptions, "onRequestOptions");
  }
});
async function createOrUpdateGoogleUser(env, googleUser) {
...
async function createUserSession2(env, user) {
  const sessionToken = crypto.randomUUID() + "_" + Math.random().toString(36).substr(2, 16);
  const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3).toISOString();
  const session = {
    userId: user.id,
...
__name(createUserSession2, "createUserSession2");
function getCurrentDomain2(request, env) {
  const requestUrl = new URL(request.url);
  const hostname = requestUrl.hostname;
  const allowedDomains = [
    "svtr.ai",
...
__name(getCurrentDomain2, "getCurrentDomain2");
async function onRequestGet2(context) {
  try {
    ...
    const error = url.searchParams.get("error");
    const state = url.searchParams.get("state");
    const currentDomain = getCurrentDomain2(request, env);
      ...
      try {
        ...
        if (state) {
          try {
            const stateData2 = JSON.parse(atob(state));
            originalDomain2 = stateData2.originalDomain || currentDomain;
            console.log("Google OAuth\u56DE\u8C03\uFF0C\u539F\u59CB\u57DF\u540D:", originalDomain2);
          } catch (e) {
            console.warn("\u65E0\u6CD5\u89E3\u6790state\u53C2\u6570:", e);
          }
        }
        const unifiedCallbackDomain2 = env.APP_URL || "https://svtr.ai";
        const tokenResponse = await fetch("https://oauth2.googleapis.com/token", {
          ...
          body: new URLSearchParams({
            ...
            redirect_uri: `${unifiedCallbackDomain2}/api/auth/google`
          })
        });
        if (!tokenResponse.ok) {
          const errorText = await tokenResponse.text();
        ...
        const sessionToken = await createUserSession2(env, user);
        const redirectUrl = new URL(originalDomain2);
        redirectUrl.searchParams.set("auth_success", "true");
        redirectUrl.searchParams.set("token", sessionToken);
        redirectUrl.searchParams.set("user", JSON.stringify({
        ...
        console.error("Google OAuth\u56DE\u8C03\u5904\u7406\u5931\u8D25:", error2);
        return Response.redirect(`${originalDomain}?auth_error=callback_failed`);
      }
    }
...
var init_google = __esm({
  "api/auth/google.ts"() {
    ...
    __name2(getCurrentDomain2, "getCurrentDomain");
...
__name(getLinkedInEmail, "getLinkedInEmail");
async function createOrUpdateLinkedInUser(env, linkedinUser, email) {
  const existingUserData = await env.SVTR_CACHE.get(`user_${email}`);
  let avatar = "";
...
async function createUserSession3(env, user) {
  const sessionToken = crypto.randomUUID() + "_" + Math.random().toString(36).substr(2, 16);
  const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3).toISOString();
  const session = {
    userId: user.id,
...
__name(createUserSession3, "createUserSession3");
function getCurrentDomain3(request, env) {
  const requestUrl = new URL(request.url);
  const hostname = requestUrl.hostname;
  const allowedDomains = [
    "svtr.ai",
...
__name(getCurrentDomain3, "getCurrentDomain3");
async function onRequestGet3(context) {
  try {
    ...
    const error = url.searchParams.get("error");
    const state = url.searchParams.get("state");
    const currentDomain = getCurrentDomain3(request, env);
      ...
      try {
        ...
        if (state) {
          try {
            const stateData2 = JSON.parse(atob(state));
            originalDomain2 = stateData2.originalDomain || currentDomain;
            console.log("LinkedIn OAuth\u56DE\u8C03\uFF0C\u539F\u59CB\u57DF\u540D:", originalDomain2);
          } catch (e) {
            console.warn("\u65E0\u6CD5\u89E3\u6790state\u53C2\u6570:", e);
          }
        }
        ...
        const tokenResponse = await fetch("https://www.linkedin.com/oauth/v2/accessToken", {
          ...
          body: new URLSearchParams({
            ...
            redirect_uri: `${unifiedCallbackDomain2}/api/auth/linkedin`
          })
        });
        if (!tokenResponse.ok) {
          const errorText = await tokenResponse.text();
        ...
        const sessionToken = await createUserSession3(env, user);
        const redirectUrl = new URL(originalDomain2);
        redirectUrl.searchParams.set("auth_success", "true");
        redirectUrl.searchParams.set("token", sessionToken);
        redirectUrl.searchParams.set("user", JSON.stringify({
        ...
        console.error("LinkedIn OAuth\u56DE\u8C03\u5904\u7406\u5931\u8D25:", error2);
        return Response.redirect(`${originalDomain}?auth_error=callback_failed`);
      }
    }
...
async function createOrUpdateUser(env, email, userData) {
  const existingUserData = await env.SVTR_CACHE.get(`user_${email}`);
  if (existingUserData) {
    const existingUser = JSON.parse(existingUserData);
    const updatedUser = {
...
async function createUserSession4(env, user) {
...
__name(createUserSession4, "createUserSession4");
async function onRequestPost(context) {
  try {
    ...
    if (action === "send_code") {
      ...
      if (lastSent && Date.now() - parseInt(lastSent) < 6e4) {
        return new Response(JSON.stringify({
          success: false,
...
var init_auth = __esm({
  "api/auth.ts"() {
    ...
    __name2(createOrUpdateUser, "createOrUpdateUser");
    __name2(createUserSession4, "createUserSession");
...
\u4E25\u683C\u57FA\u4E8ESVTR AI\u521B\u6295\u5E93\u63D0\u4F9B\u7684\u77E5\u8BC6\u5185\u5BB9\uFF0C\u7ED9\u51FA\u51C6\u786E\u3001\u5BA2\u89C2\u7684\u6570\u636E\u56DE\u590D\u3002`;
    __name2(shouldShowSourceInfo, "shouldShowSourceInfo");
    __name2(isBusinessInquiry, "isBusinessInquiry");
...
var init_company_analysis_formatter = __esm({
  "lib/company-analysis-formatter.ts"() {
    ...
    CompanyAnalysisFormatter = class {
      ...
      static {
        __name2(this, "CompanyAnalysisFormatter");
      }
      /**
       * 格式化公司分析报告
       */
...
**\u26A1 \u6570\u636E\u66F4\u65B0**\uFF1A\u5305\u542B\u6700\u65B0\u5E02\u573A\u4FE1\u606F`;
    ...
    __name2(extractCompanyDataFromRAG, "extractCompanyDataFromRAG");
    __name2(createCompanyAnalysisFormatter, "createCompanyAnalysisFormatter");
  }
});
...
var init_chat_optimized = __esm({
  "api/chat-optimized.ts"() {
    ...
    ENHANCED_SYSTEM_PROMPT = `\u4F60\u662F\u51EF\u745E(Kerry)\uFF0C\u7845\u8C37\u79D1\u6280\u8BC4\u8BBA(SVTR)\u7684AI\u521B\u6295\u5206\u6790\u5E08\uFF0C\u4E13\u6CE8\u4E8E\u4E3A\u7528\u6237\u63D0\u4F9B\u51C6\u786E\u3001\u6709\u7528\u7684AI\u521B\u6295\u4FE1\u606F\u3002
...
\u4F7F\u7528GPT-OSS\u5F00\u6E90\u6A21\u578B\u7684\u5F3A\u5927\u63A8\u7406\u80FD\u529B\u548C\u5B9E\u65F6\u7F51\u7EDC\u641C\u7D22\uFF0C\u76F4\u63A5\u63D0\u4F9B\u6709\u4EF7\u503C\u7684\u4E13\u4E1A\u56DE\u7B54\u3002`;
    __name2(enhanceUserQuery, "enhanceUserQuery");
    __name2(generateSmartPrompt, "generateSmartPrompt");
...
var init_feishu_webhook = __esm({
  "api/feishu-webhook.ts"() {
    ...
    __name2(handleDocumentCreated, "handleDocumentCreated");
    __name2(handleDocumentUpdated, "handleDocumentUpdated");
    __name2(handleDocumentDeleted, "handleDocumentDeleted");
    __name2(logContentEvent, "logContentEvent");
  ...
  try {
    ...
    if (action === "stats") {
      ...
      const stats = {
        ...
        categoriesBreakdown: projects.reduce((acc, p) => {
...
var ConversationContextManager;
var init_conversation_context = __esm({
  "lib/conversation-context.ts"() {
    ...
    ConversationContextManager = class {
      ...
      static {
        __name2(this, "ConversationContextManager");
      }
...
var routes;
var init_functionsRoutes_0_39491537850192104 = __esm({
  "../.wrangler/tmp/pages-cVgoM6/functionsRoutes-0.39491537850192104.mjs"() {
    "use strict";
    init_github();
    init_github();
    init_google();
...
// .wrangler/tmp/pages-cVgoM6/slbhi2cnqu.js
...
// .wrangler/tmp/bundle-5j1QCX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-5j1QCX/middleware-loader.entry.ts
...
//# sourceMappingURL=slbhi2cnqu.js.map
